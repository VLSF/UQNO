...
def get_laplacian(model, x, B, features):
    return grad(lambda x: grad(model, argnums=0)(x, B, features)[0])(x)[0] + \
           grad(lambda x: grad(model, argnums=0)(x, B, features)[1])(x)[1]


def compute_loss_data(model, coordinates, B, a, dx_a, dy_a, rhs, features, targets):
    sol = vmap(model, in_axes=(0, None, None))(coordinates, B, features)
    loss = (sol - targets)
    return jnp.linalg.norm(loss)


def compute_loss_residual(model, coordinates, B, a, dx_a, dy_a, rhs, features):
...
def compute_loss(model, coordinates, B, a, dx_a, dy_a, rhs, features, targets):
    return jnp.mean(
        vmap(compute_loss_residual, in_axes=(None, None, None, 0, 0, 0, 0, 0))(model, coordinates, B, a, dx_a, dy_a,
                                                                               rhs,features))
        #                                                                         + jnp.mean(
        # vmap(compute_loss_data, in_axes=(None, None, None, 0, 0, 0, 0, 0, 0))(model, coordinates, B, a, dx_a, dy_a, rhs,
        #                                                                       features, targets))
...
def make_step(model, coordinates, B, a, dx_a, dy_a, rhs, features, optim, opt_state, targets):
    loss, grads = compute_loss_and_grads(model, coordinates, B, a, dx_a, dy_a, rhs, features, targets)
...
def make_step_scan(carry, ind, optim, M):
    model, coordinates, B, a, dx_a, dy_a, rhs, features, targets, opt_state = carry
    loss, grads = compute_loss_and_grads(model, coordinates[ind[:M]], B, a[ind[M:], :][:, ind[:M]],
                                         dx_a[ind[M:], :][:, ind[:M]], dy_a[ind[M:], :][:, ind[:M]],
                                         rhs[ind[M:], :][:, ind[:M]], features[ind[M:]],
                                         targets[ind[M:], :][:, ind[:M]])
    updates, opt_state = optim.update(grads, opt_state, eqx.filter(model, eqx.is_array))
    model = eqx.apply_updates(model, updates)
    return [model, coordinates, B, a, dx_a, dy_a, rhs, features, targets, opt_state], loss
...
# features = jnp.concatenate([a_train,rhs_train],1)
# normalizer=sol_train.max()
# sol_train=sol_train/normalizer
# sol_eval=sol_eval/normalizer
...
inds = random.choice(key, N_mesh * N_mesh, (N_epochs, M))
inds_ = random.choice(key, N_train, (N_epochs, N_batch))
inds = jnp.concatenate([inds, inds_], 1)

N_features = [2 * N_fourier_features, 100, 30]
N_layers = 6
...
carry = [model, coords_train, B, a_train, dx_a_train, dy_a_train, rhs_train, features, sol_train, opt_state]
