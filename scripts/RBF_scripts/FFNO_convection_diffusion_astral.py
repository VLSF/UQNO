...
class FFNO_RBF(eqx.Module):
    ...
    def __call__(self, x_point, u, x, phi_k, k, a, sigma):
...
class FFNO_RBF2(eqx.Module):
    ...
    def __init__(self, N_layers, N_features, N_modes, key, D=1, sigma=1.0):
        ...
        self.models = [
            FFNO_RBF_u(N_layers, N_features, N_modes, keys[0], D=D),
    ...
    def __call__(self, x_point, u, x, phi_k, k, a, i):
        return self.models[i](x_point, u, x, phi_k, k, a, self.sigma)
    
def compute_loss_(model, coordinates, phi_k, k, f, a, features, coords):
    du = vmap(lambda x: grad(model, argnums=0)(x, features, coords, phi_k, k, a, 1))(coordinates)
    dy_dx = vmap(lambda x: grad(model, argnums=0)(x, features, coords, phi_k, k, a, 0))(coordinates)[:, 0]
    y = vmap(lambda x: model(x, features, coords, phi_k, k, a, 0))(coordinates)
    r = f - du[:, 1] - a*du[:, 0] + dy_dx
    l = y - du[:, 0]
    loss = jnp.linalg.norm(r) / jnp.pi + jnp.linalg.norm(l)
    return loss
...
def compute_upper_bound(model, coordinates, features, coords, weights, phi_k, k, f, a, T):
    du = vmap(lambda x: grad(model, argnums=0)(x, features, coords, phi_k, k, a, 1))(coordinates)
    dy_dx = vmap(lambda x: grad(model, argnums=0)(x, features, coords, phi_k, k, a, 0))(coordinates)[:, 0]
    y = vmap(lambda x: model(x, features, coords, phi_k, k, a, 0))(coordinates)
    r = f - du[:, 1] - a*du[:, 0] + dy_dx
    l = y - du[:, 0]
    ub = jnp.sqrt(jnp.sum(jnp.sum((l**2).reshape(weights.size, weights.size)*weights, axis=1) * weights[0])*T/4)
    ub += jnp.sqrt(jnp.sum(jnp.sum((r**2).reshape(weights.size, weights.size)*weights, axis=1) * weights[0])*T/4) / jnp.pi
    return ub
...
def compute_error_energy_norm(model, coordinates, phi_k, k, a, features, coords, d_exact_sol, exact_sol, T, weights):
    ...
    u_T = vmap(lambda x: model(x, features, coords, phi_k, k, a, 1))(coordinates.reshape(weights.size, weights.size, 2)[:, -1, :].reshape(-1, 2))
...
def compute_relative_error(model, coords_eval, target_eval, phi_k, k, a, features, coords):
    prediction = vmap(model, in_axes=(0, None, None, None, None, None, None))(coords_eval, features, coords, phi_k, k, a, 1)
...
if __name__ == "__main__":
    ...
    for N_updates, Nx, N_batch, N_drop, N_modes, N_layers, learning_rate, gamma, N_h_features in itertools.product(*iterators):
        ...
        compute_upper_bound_scan = lambda carry, ind: (carry, compute_upper_bound(carry[0], carry[1], carry[2][ind], carry[3], carry[4], carry[5][ind], carry[6], carry[7][ind], carry[8][ind], carry[9]))
        _, upper_bounds = scan(compute_upper_bound_scan, [model, coords_leg, features, coords_train_, weights, phi_k, k, f_leg, a, T], inds)
