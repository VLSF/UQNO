...
class FFNO_RBF3(eqx.Module):
    ...
    def __init__(self, N_layers, N_features, N_modes, key, D=1, sigma=1.0):
        ...
        self.models = [
            ...
            FFNO_RBF(N_layers, N_features, N_modes, keys[1], D=D)
        ]
...
def compute_loss_(model, coordinates, mu, f_x, f_y, features, coords):
    ...
    w = vmap(model, in_axes=(0, None, None, None))(coordinates, features, coords, 2)
    dw = vmap(lambda x: grad(model, argnums=0)(x, features, coords, 2), in_axes=0, out_axes=1)(coordinates)
    loss = jnp.sum((f_x - E_x - dw[1])**2 + (f_y - E_y + dw[0])**2 + (w - mu*curl)**2 / mu)
    return loss

def compute_loss(model, coordinates, mu, f_x, f_y, features, coords):
    return jnp.mean(vmap(compute_loss_, in_axes=(None, None, 0, 0, 0, 0, None))(model, coordinates, mu, f_x, f_y, features, coords))
...
def compute_upper_bound(model, coordinates, mu, f_x, f_y, features, coords, weights):
    curl = vmap(get_curl, in_axes=(None, 0, None, None))(model, coordinates, features, coords)
    dxdy_E_x, dxdy_E_y = vmap(get_mixed, in_axes=(None, 0, None, None))(model, coordinates, features, coords)
    d2y_E_x, d2x_E_y = vmap(get_second, in_axes=(None, 0, None, None))(model, coordinates, features, coords)
    E_x = vmap(model, in_axes=(0, None, None, None))(coordinates, features, coords, 0)
...
def make_step_scan(carry, ind, optim, M):
    model, coordinates, mu, f_x, f_y, features, coords, opt_state = carry
    loss, grads = compute_loss_and_grads(model, coordinates[ind[:M]], mu[ind[M:], :][:, ind[:M]], f_x[ind[M:], :][:, ind[:M]], f_y[ind[M:], :][:, ind[:M]], features[ind[M:]], coords)
    updates, opt_state = optim.update(grads, opt_state, eqx.filter(model, eqx.is_array))
    model = eqx.apply_updates(model, updates)
    return [model, coordinates, mu, f_x, f_y, features, coords, opt_state], loss
...
if __name__ == "__main__":
    ...
    for N_updates, Nx, N_batch, N_drop, N_modes, N_layers, learning_rate, gamma, N_h_features in itertools.product(*iterators):
        ...
        carry = [model, coords_train, mu_train, f_x_train, f_y_train, features, coords_train_, opt_state]
        ...
        compute_upper_bound_scan = lambda carry, ind: (carry, compute_upper_bound(carry[0], carry[1], carry[2][ind], carry[3][ind], carry[4][ind], carry[5][ind], carry[6], carry[7]))
        _, upper_bounds = scan(compute_upper_bound_scan, [model, coords_legendre, mu_legendre, f_x_legendre, f_y_legendre, features, coords_train_, weights], inds)
