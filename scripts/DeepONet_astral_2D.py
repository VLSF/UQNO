...
def get_sample(key, coords_train, weights_, coords_legendre, coords_eval):
    ...
    a_eval = a(coords_eval.T).T
    rhs_eval = rhs(coords_eval.T).T
    sol_eval = sol(coords_eval.T).T

    return a_train, rhs_train, dx_a_train, dy_a_train, a_eval_legendre, dx_sol_eval_legendre, dy_sol_eval_legendre, rhs_legendre, a_legendre,a_eval,rhs_eval, sol_eval, C_F

coords_train, weights_, coords_legendre, coords_eval = get_coordinates()
a_train, rhs_train, dx_a_train, dy_a_train, a_eval_legendre, dx_sol_eval_legendre, dy_sol_eval_legendre, rhs_legendre, a_legendre,a_eval,rhs_eval, sol_eval, C_F = [], [], [], [], [], [], [], [], [], [], [],[],[]
key = random.PRNGKey(23)
keys = random.split(key, 100)
for key in keys:
    a_train_, rhs_train_, dx_a_train_, dy_a_train_, a_eval_legendre_, dx_sol_eval_legendre_, dy_sol_eval_legendre_, rhs_legendre_, a_legendre_,a_eval_,rhs_eval_, sol_eval_, C_F_ = get_sample(key, coords_train, weights_, coords_legendre, coords_eval)
    ...
    a_eval.append(a_eval_)
    rhs_eval.append(rhs_eval_)
...
rhs_eval = jnp.stack(rhs_eval)
a_eval = jnp.stack(a_eval)
C_F = jnp.array(C_F)
...
class DeepONet3(eqx.Module):
    ...
    def __init__(self, N_features, N_layers, N_branch, key):
        ...
        self.models.append(DeepONet(N_features, N_layers, N_branch, keys[-1]))
        self.beta = jnp.array([1.0,])

    def __call__(self, x, B, features, i):
        return self.models[i](x, B, features)

def compute_loss_(model, coordinates, rhs, a, B, C_F, features):
    flux = vmap(grad(model, argnums=0), in_axes=(0, None, None, None), out_axes=1)(coordinates, B, features, 2)
    dx_y = [vmap(grad(model, argnums=0), in_axes=(0, None, None, None), out_axes=1)(coordinates, B, features, i)[i] for i in [0, 1]]
...
def compute_loss(model, coordinates, rhs, a, B, C_F, features):
    return jnp.mean(vmap(compute_loss_, in_axes=(None, None, 0, 0, None, 0, 0))(model, coordinates, rhs, a, B, C_F, features))
...
def make_step_scan(carry, ind, optim, M):
    model, coordinates, rhs, a, B, C_F, features, opt_state = carry
    loss, grads = compute_loss_and_grads(model, coordinates[ind[:M]], rhs[ind[M:], :][:, ind[:M]], a[ind[M:], :][:, ind[:M]], B, C_F[ind[M:]], features[ind[M:]])
    updates, opt_state = optim.update(grads, opt_state, eqx.filter(model, eqx.is_array))
    model = eqx.apply_updates(model, updates)
    return [model, coordinates, rhs, a, B, C_F, features, opt_state], loss
...
carry = [model, coords_train, rhs_train, a_train, B, C_F, features, opt_state]
...
for i in range(10):
    ...
    predicted = vmap(lambda x: vmap(model, in_axes=(0, None, None, None))(coords_eval, B, x, 2))(features)
    relative_error = jnp.linalg.norm(predicted - sol_eval, axis=1) / jnp.linalg.norm(sol_eval, axis=1)
    energy_norm = jnp.sqrt(vmap(compute_error_energy_norm, in_axes=(None, None, 0, 0, 0, None, None, 0))(model, coords_legendre, a_eval_legendre, dx_sol_eval_legendre, dy_sol_eval_legendre, weights_, B, features))
    upper_bound = jnp.sqrt(vmap(compute_upper_bound, in_axes=(None, None, None, 0, 0, None, 0, 0))(model, coords_legendre, weights_, rhs_legendre, a_legendre, B, C_F, features))

    print(f"Epochs {(i+1)*N_epochs}, Train errors {jnp.mean(relative_error[:N_train])} +/- {jnp.sqrt(jnp.var(relative_error[:N_train]))}, {jnp.mean(energy_norm[:N_train])} +/- {jnp.sqrt(jnp.var(energy_norm[:N_train]))}, {jnp.mean(upper_bound[:N_train])} +/- {jnp.sqrt(jnp.var(upper_bound[:N_train]))}")
    print(f"Epochs {(i+1)*N_epochs}, Test errors {jnp.mean(relative_error[N_train:])} +/- {jnp.sqrt(jnp.var(relative_error[N_train:]))}, {jnp.mean(energy_norm[N_train:])} +/- {jnp.sqrt(jnp.var(energy_norm[N_train:]))}, {jnp.mean(upper_bound[N_train:])} +/- {jnp.sqrt(jnp.var(upper_bound[N_train:]))}")
