...
def compute_loss(model, input, target, x):
    output = vmap(lambda z: model(z, x), in_axes=(0,))(input)
...
@eqx.filter_jit
def make_step(model, input, target, x, optim, opt_state):
    # for optimizers that require model for update, e.g. adamw
    loss, grads = compute_loss_and_grads(model, input, target, x)
