...
def get_sample(key, coords_train, weights_, coords_legendre, coords_eval):
    ...
    f = - (a * phi.diff("x")).diff("x") - (a * phi.diff("y")).diff("y")-eps*((a * phi.diff("y")).diff("x")+(a * phi.diff("x")).diff("y"))
...
if __name__ == "__main__":
    ...
    jnp.savez("/mnt/local/data/tianchi.yu/UQNO/elliptic" + str(eps) + ".npz", **data)
